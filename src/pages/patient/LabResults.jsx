    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Biocheminiai rodikliai ir lipidograma • 2026-07-15</p>
        </div>
        <Badge className="bg-sky-50 text-sky-700 border-sky-200"><FlaskConical className="w-3 h-3 mr-1" /> 12 rodiklių</Badge>
      </div>
          ...
          return (
            <Card key={i} className={`p-5 rounded-2xl ${isOutOfRange ? "border-amber-200" : "border-slate-100"}`}>
              ...
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-400">Norma: {r.refLow}–{r.refHigh}</span>
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-slate-400">
                    {r.trend === "up" ? "kilimas" : r.trend === "down" ? "kritimas" : "stabili"}
                  </span>
                </div>
              </div>
              {/* Range bar */}
