...
const specializations = ["Visos", "Šeimos medicina", "Kardiologija", "Endokrinologija", "Vidaus ligos", "Profilaktinė medicina", "Gyvensenos medicina"];

export default function Marketplace() {
  const [spec, setSpec] = useState("Visos");
  const [search, setSearch] = useState("");

  const filtered = doctors.filter(d =>
    (spec === "Visos" || d.specialization === spec) &&
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Raskite savo gydytoją</h1>
        <p className="text-slate-500 mt-1">Pasirinkite prižiūrintį gydytoją nuolatinei sveikatos priežiūrai</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Ieškoti pagal vardą ar specializaciją..."
            ...
            <div className="p-5">
              ...
              <div className="grid grid-cols-2 gap-2 mt-4 text-xs">
                <div className="flex items-center gap-1.5 text-slate-500">
                  <Clock className="w-3.5 h-3.5" /> {doc.experience} m. patirtis
                </div>
                <div className="flex items-center gap-1.5 text-slate-500">
                ...
                <div className="flex items-center gap-1.5 text-slate-500">
                  <Stethoscope className="w-3.5 h-3.5" /> {doc.activePatients}/{doc.capacity} pacientai
                </div>
              </div>
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-50">
                <div>
                  <p className="text-xs text-slate-400">Nuo</p>
                  <p className="text-lg font-bold text-slate-800">€{doc.price}<span className="text-xs font-normal text-slate-400">/mėn.</span></p>
                </div>
                {doc.available ? (
                  <Button size="sm" className="bg-sky-600 hover:bg-sky-700">Pasirinkti</Button>
                ) : (
                  <Badge className="bg-slate-100 text-slate-500 border-slate-200">Nepriima</Badge>
                )}
              </div>
            </div>
          </Card>
