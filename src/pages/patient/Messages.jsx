...
export default function Messages() {
  ...
  const send = () => {
    ...
    setMsgs([...msgs, { id: `msg${Date.now()}`, from: "patient", text: input, time: new Date().toLocaleString("lt-LT") }]);
    setInput("");
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Žinutės</h1>
        <p className="text-slate-500 mt-1">Saugus susirašinėjimas su Dr. Aiste Kazlauskieni</p>
      </div>

      <Card className="p-3 rounded-2xl bg-amber-50/30 border-amber-100">
        <div className="flex items-center gap-2 px-2">
          <Clock className="w-4 h-4 text-amber-600" />
          <p className="text-sm text-slate-600">Numatomas atsakymo laikas: per 24 valandas. Žinučių sistema nėra skubiosios pagalbos kanalas.</p>
        </div>
      </Card>
