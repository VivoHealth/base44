...
const sources = ["All", "OMRON", "Manual entry"];

export default function MeasurementsEn() {
  ...
  const metrics = [
    { icon: Heart, label: "Blood pressure", value: `${latest?.systolic}/${latest?.diastolic}`, unit: "mmHg", color: "rose", active: true },
    { icon: Activity, label: "Pulse", value: avgPulse, unit: "bpm", color: "sky" },
    { icon: Scale, label: "Weight", value: 86, unit: "kg", color: "violet" },
    { icon: Droplet, label: "Glucose", value: 5.6, unit: "mmol/L", color: "amber" },
    { icon: Activity, label: "O2 saturation", value: 97, unit: "%", color: "emerald" },
    { icon: Thermometer, label: "Temperature", value: 36.6, unit: "°C", color: "orange" },
    { icon: Moon, label: "Sleep", value: 6.5, unit: "hrs", color: "indigo" },
    { icon: Footprints, label: "Physical activity", value: 4231, unit: "steps", color: "emerald" },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Measurements</h1>
        <p className="text-slate-500 mt-1">Your health metrics and trends</p>
      </div>
        ...
        {metrics.map((m, i) => (
          <Card key={i} className={`p-4 rounded-2xl ${m.active ? "border-rose-200 bg-rose-50/30" : "border-slate-100"}`}>
            <div className="flex items-center justify-between mb-2">
              ...
              {m.active && <Badge className="bg-rose-50 text-rose-700 border-rose-200">Active</Badge>}
            </div>
      ...
      <Card className="p-6 rounded-2xl border-slate-100">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h3 className="font-bold text-slate-800">Blood pressure chart</h3>
            <div className="flex gap-4 mt-1 text-sm text-slate-500">
              <span>Avg. systolic: <strong className="text-slate-700">{avgSys}</strong></span>
              <span>Avg. diastolic: <strong className="text-slate-700">{avgDia}</strong></span>
              <span>Avg. pulse: <strong className="text-slate-700">{avgPulse}</strong></span>
            </div>
          </div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={chartData}>
            ...
            <Area type="monotone" dataKey="sistolinis" stroke="#0ea5e9" strokeWidth={2} fill="url(#sysGrad)" name="Systolic" />
            <Line type="monotone" dataKey="diastolinis" stroke="#10b981" strokeWidth={2} dot={false} name="Diastolic" />
            <Line type="monotone" dataKey="pulsas" stroke="#8b5cf6" strokeWidth={1.5} dot={false} strokeDasharray="4 4" name="Pulse" />
          </AreaChart>
        </ResponsiveContainer>
      </Card>
      ...
      {/* Recent readings */}
      <Card className="p-6 rounded-2xl border-slate-100">
        <h3 className="font-bold text-slate-800 mb-4">Recent measurements</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-slate-400 border-b border-slate-100">
                ...
                <th className="pb-3 font-medium">Systolic</th>
                <th className="pb-3 font-medium">Diastolic</th>
