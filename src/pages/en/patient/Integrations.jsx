...
const statusConfig = {
  ...
  available: { label: "Available", color: "bg-sky-50 text-sky-700 border-sky-200" },
  "coming-soon": { label: "Coming soon", color: "bg-amber-50 text-amber-700 border-amber-200" },
};
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Connect your health devices and apps</p>
      </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <Badge className={omronConnected ? statusConfig.connected.color : "bg-slate-100 text-slate-500"}>
                {omronConnected ? "Connected" : "Not connected"}
              </Badge>
              {omronConnected && <Badge variant="outline" className="text-xs">Demo</Badge>}
            </div>
            <p className="text-sm text-slate-500 mt-1">Blood pressure monitors – automatic measurement import</p>
            ...
            {omronConnected ? (
              <div className="mt-4 grid sm:grid-cols-2 gap-4">
                ...
                <div className="p-3 rounded-xl bg-slate-50/50">
                  <p className="text-xs text-slate-500">Imported measurements</p>
                  <p className="text-sm font-medium text-slate-800">48 measurements</p>
                </div>
                <div className="p-3 rounded-xl bg-slate-50/50">
                  <p className="text-xs text-slate-500">Account</p>
                  <p className="text-sm font-medium text-slate-800">jonas.petrauskas@email.lt</p>
                </div>
                <Button variant="outline" size="sm" className="sm:col-span-1"><RefreshCw className="w-4 h-4 mr-1" /> Sync now</Button>
              ...
              <Button onClick={handleConnect} disabled={connecting} className="mt-4 bg-sky-600 hover:bg-sky-700">
                {connecting ? "Connecting..." : "Connect OMRON account"}
              </Button>
            )}
            {connecting && (
              <p className="text-xs text-slate-400 mt-2">Simulating OAuth login (demo)...</p>
            )}
          </div>
        </div>
      </Card>
          ...
          return (
            <Card key={i} className="p-5 rounded-2xl border-slate-100">
              ...
              {int.status === "available" && (
                <Button variant="outline" size="sm" className="w-full mt-3 text-xs">Connect</Button>
              )}
