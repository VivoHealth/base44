...
const alertLabels = { urgent: "Urgent", priority: "Priority", review: "Review", info: "Info" };

export default function PatientOverviewEn() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Hello, Jonas 👋</h1>
        <p className="text-slate-500 mt-1">Here's your health overview for today, {new Date().toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" })}</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 rounded-2xl border-slate-100">
          ...
          <p className="text-sm font-semibold text-slate-800">Monitored</p>
        </Card>
        <Card className="p-4 rounded-2xl border-slate-100">
          <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-xs text-slate-500">Chosen doctor</p>
        ...
        <Card className="p-4 rounded-2xl border-slate-100">
          ...
          <p className="text-xs text-slate-500">Latest blood pressure</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 rounded-2xl border-slate-100">
          ...
          <p className="text-xs text-slate-500">Latest lab test</p>
        ...
        <Card className="p-4 rounded-2xl border-slate-100">
          ...
          <p className="text-sm font-semibold text-slate-800">{medications.filter(m => m.active).length} medications</p>
        </Card>
        <Card className="p-4 rounded-2xl border-slate-100">
          <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-sm font-semibold text-slate-800">3 actions</p>
        </Card>
        <Card className="p-4 rounded-2xl border-slate-100">
          <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-sm font-semibold text-slate-800">€39/mo</p>
        </Card>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-6 rounded-2xl border-slate-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/en/patient/measurements"><Button variant="outline" size="sm">View <ChevronRight className="w-4 h-4" /></Button></Link>
          </div>
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={bpChart}>
              ...
              <Area type="monotone" dataKey="sistolinis" stroke="#0ea5e9" strokeWidth={2} fill="url(#sist)" name="Systolic" />
              <Area type="monotone" dataKey="diastolinis" stroke="#10b981" strokeWidth={2} fill="none" name="Diastolic" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card className="p-6 rounded-2xl border-slate-100">
          ...
          {alerts[0]?.level === "urgent" && (
            <div className="mt-3 p-3 rounded-xl bg-rose-50 border border-rose-200 text-xs text-rose-700">
              ...
              This information may require urgent medical attention. MyHealthPilot is not an emergency service. Contact emergency services when immediate care is needed.
            </div>
          )}
        </Card>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6 rounded-2xl border-slate-100 bg-gradient-to-br from-sky-50/50 to-emerald-50/50">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-bold text-slate-800">AI Health Assistant</h3>
              <p className="text-xs text-slate-500">Weekly summary ready</p>
            </div>
          </div>
          <p className="text-sm text-slate-600 mb-4">
            Your weekly summary is ready. Average blood pressure rose 3 mmHg. Cholesterol exceeded the normal range.
          </p>
          <Link to="/en/patient/ai"><Button className="bg-sky-600 hover:bg-sky-700" size="sm">Ask AI <ChevronRight className="w-4 h-4" /></Button></Link>
        </Card>
        ...
        <Card className="p-6 rounded-2xl border-slate-100">
          ...
          <div className="space-y-2">
            {[
              { text: "Mark Atorvastatin dose", icon: Pill, color: "violet" },
              { text: "Review lab results", icon: FlaskConical, color: "sky" },
              { text: "Approve AI expanded data", icon: FileText, color: "emerald" },
            ].map((a, i) => (
      ...
      {/* Timeline */}
      <Card className="p-6 rounded-2xl border-slate-100">
        <h3 className="font-bold text-slate-800 mb-4">Health events timeline</h3>
